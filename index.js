...
io.sockets.on('connection', (socket) => {
  ...
  let body2 = world.createBody({
	...
	var boxBody = Bodies.rectangle(1500, 0, 25, 25, {
		density: 1.0,
		friction: 0.8,
		restitution: 0.2
	});
...
function tick() {
	const intervalId = setInterval(() => {
		...
		for (let key of Object.keys(playerVitals)) {
			...
			Matter.Body.applyForce(players[key], {x: playerVitals[key].x, y: playerVitals[key].y}, {x: force.x + force2.x, y: force.y + force2.y});
